...
export const login = async (req, res) => {
  try {
    ...
    if (!user || error) {
      ...
      await logLoginAttempt(null, username.trim(), 'failed_not_found', req.clientIp);
    ...
    if (!user.is_active) {
      await logLoginAttempt(user.id, username.trim(), 'failed_inactive', req.clientIp);
    ...
    if (!passwordMatch) {
      await logLoginAttempt(user.id, username.trim(), 'failed_password', req.clientIp);
    ...
    await logLoginAttempt(user.id, username.trim(), 'success', req.clientIp, req.get('user-agent'));
...
// ============================================
// GET ALL USERS (for syncing Venn diagram & visualizers)
// ============================================
...
export const getUsers = async (req, res) => {
  try {
    const { data: users, error } = await supabase
      .from('users')
      .select('id, username, name, email, roles, is_active, avatar_url, created_at')
